...
public class PNAnimation3 extends Animation {
	...
	public PNAnimation3(UIPluginContext context, Petrinet pn, XLog log,Marking initMarking) {
	...
	private void updatemapping(Petrinet net, Marking initMarking,
		...
		Map<Arc, Arc> arcarc = new HashMap<Arc, Arc>();

		Map<Place, Vector<Place>> placeplace = new HashMap<Place, Vector<Place>>();
		...
		for (Place p : newmarking.baseSet()){
			...
			placetime.put(p, pre);
			placecollarc.put(p, new Vector<Arc>());
			...
			if(!t.isInvisible()){
				if(t.getLabel().endsWith("start")){
					...
					for (PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode> edge : inedge) {
						...
						if(placecollarc.containsKey(p)){
							Vector<Arc> collarcs = placecollarc.get(p);
							collarcs.add(arc);
							long temp = placetime.get(p);
							placetime.remove(p);
							newmarking.remove(p);
							int tot = collarcs.size();
							long delta = (timeStartTrace-temp)/(tot);
							int i=0;
							for(Arc arco : collarcs){
								long  x = temp+(delta*i);

								i++;
								ArcTime arctime = new ArcTime(arco, x);
								arctime.setEndDeltaTime(delta);
								//if(!this.esistearco(collArcTime,arco)){
								collArcTime.add(arctime); 
								//}else{

								//}
								if(arcarc.containsKey(arco)){
									Arc g = arcarc.get(arco);
									...
									if (target instanceof Place){
										...
										placetime.remove(pla);
										placetime.put(pla, x+delta);
									}

								}
								PetrinetNode target = arco.getTarget();
								if (target instanceof Place){
									Place pla = (Place) target;

									/*if(placeplace.containsKey(pla)  ){
										Vector<Place> paralplaces = placeplace.get(pla);
										for(Place paralplace: paralplaces ){
											placetime.remove(paralplace);
											placetime.put(paralplace, x);
										}
									}*/
								}
							}
				...
				if(t.getLabel().endsWith("complete")){
					...
					for (PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode> edge : outedge) {
						...
						Place p = (Place)arc.getTarget();
						newmarking.add(p);
						placetime.put(p, timeEndTrace);
						Vector<Arc> collarcs = new Vector<Arc>();
						collarcs.add(arc);
						placecollarc.put(p,collarcs);
					}
				...
				Collection<PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode>> outedge = t.getGraph().getOutEdges(t);
				Collection<PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode>> inedge = t.getGraph().getInEdges(t);
				Vector<Arc> collarcs = null;
				long temp = 0;
				for(PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode> edge : inedge){
					Arc arc = (Arc) edge;
					Place p = (Place)arc.getSource();
					if(placecollarc.containsKey(p)){
						collarcs = placecollarc.get(p);
						collarcs.add(arc);

						placecollarc.remove(p);
						newmarking.remove(p);
					}
					if(placetime.containsKey(p)){
						temp =  placetime.get(p);
						placetime.remove(p);
					}else{
				...
				for(PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode> edge : outedge){
					...
					/*	if(collarcs==null){	
				        collarcs = new Vector<Arc>();
						collarcs.add(arc);
						placecollarc.put(p, collarcs);

					}else{
						Vector<Arc> tempverctor = (Vector<Arc>) collarcs.clone();
						tempverctor.add(arc);
						placecollarc.put(p,tempverctor );
					}*/
					//collarcs.clear();
					newmarking.add(p);
					placetime.put(p, temp);
					if(numout>1  && i>0 ){

						Vector<Arc> tempverctor = (Vector<Arc>) collarcs.clone();
						tempverctor.add(arc);
						arcarc.put(arc, temparc);
						placecollarc.put(p,tempverctor );
						/*collarcs = new Vector<Arc>();
						placecollarc.put(p, collarcs);
						collarcs.add(arc);
						if(!placeplace.containsKey(tempplace)){
							Vector<Place> vplace = new Vector<Place>();
							vplace.add(p);
							placeplace.put(tempplace, vplace);
						}else{
							Vector<Place> vplace =placeplace.get(tempplace);
							vplace.add(p);
						}*/
	...
	private boolean esistearco(Collection<ArcTime> collArcTime, Arc arco) {
